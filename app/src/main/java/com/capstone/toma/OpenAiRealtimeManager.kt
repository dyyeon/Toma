...
class OpenAiRealtimeManager(
    ...
    private val MODEL = "gpt-4o-realtime-preview-2024-10-01"
    ...
    private fun sendSessionUpdate() {
        val sessionUpdate = JSONObject().apply {
            ...
            put("session", JSONObject().apply {
                put("modalities", listOf("text")) // We only need intent text for cooking actions
                put("instructions", """
                    당신은 요리 보조 앱 '토마'의 비서입니다. 
                    사용자의 한국어 명령을 분석하여 반드시 다음 JSON 형식으로만 응답하세요:
                    { "intent": "NEXT_STEP" | "PREVIOUS_STEP" | "REPEAT_STEP" | "SET_TIMER" | "INGREDIENT_CHECK" | "HELP" | "CANCEL", "arguments": { "duration_min": 3 } }
